...
import { ArrowLeft, TrendingUp, DollarSign, Calendar, Target, BarChart3, Download } from "lucide-react"
...
export default function RentabilidadPage() {
  ...
  const exportarPDF = () => {
    ...
    doc.text("Análisis de Rentabilidad - Proyecto Piscícola", 20, 25)
    ...
    doc.setFontSize(14)
    doc.setTextColor(40, 40, 40)
    doc.text("Datos del Proyecto", 20, yPosition)
    yPosition += 10

    doc.setFontSize(10)
    doc.text(`Cantidad de Estanques: ${datos.cantidadEstanques}`, 20, yPosition)
    yPosition += 6
    doc.text(`Tipo de Peces: ${datos.tipoPeces.charAt(0).toUpperCase() + datos.tipoPeces.slice(1)}`, 20, yPosition)
    yPosition += 6
    doc.text(`Inversión Inicial: ${formatCurrency(datos.inversionInicial)}`, 20, yPosition)
    yPosition += 6
    doc.text(`Alevines por Estanque: ${datos.alevinosPorEstanque}`, 20, yPosition)
    yPosition += 15

    // Parámetros de rentabilidad
    doc.setFontSize(14)
    doc.text("Parámetros de Rentabilidad", 20, yPosition)
    yPosition += 10

    doc.setFontSize(10)
    doc.text(`Precio Carpa: ${formatCurrency(datos.precioCarpaKg)}/kg`, 20, yPosition)
    yPosition += 6
    doc.text(`Precio Pacu: ${formatCurrency(datos.precioPacuKg)}/kg`, 20, yPosition)
    yPosition += 6
    doc.text(`Costo Alimento: ${formatCurrency(datos.costoAlimentoKg)}/kg`, 20, yPosition)
    yPosition += 6
    doc.text(`Conversión Alimenticia: ${datos.conversionAlimenticia}`, 20, yPosition)
    yPosition += 6
    doc.text(`Costo Cal por Ciclo: ${formatCurrency(datos.costoCalCiclo)}`, 20, yPosition)
    yPosition += 6
    doc.text(`Duración del Ciclo: ${datos.duracionCicloMeses} meses`, 20, yPosition)
    yPosition += 6
    doc.text(`Supervivencia: ${(datos.supervivencia * 100).toFixed(0)}%`, 20, yPosition)
    yPosition += 15
    ...
    if (proyecciones.length > 0) {
      ...
      doc.setFontSize(10)
      doc.text(`Ganancia Total (5 años): ${formatCurrency(ultimaProyeccion.flujoAcumulado)}`, 20, yPosition)
      yPosition += 6
      doc.text(`ROI Final: ${ultimaProyeccion.roi.toFixed(1)}%`, 20, yPosition)
      yPosition += 6
      doc.text(`Punto de Equilibrio: ${puntoEquilibrio ? `Mes ${puntoEquilibrio}` : "No alcanzado"}`, 20, yPosition)
      yPosition += 6
      doc.text(`Total de Ciclos Proyectados: ${proyecciones.length}`, 20, yPosition)
      yPosition += 20

      // Tabla de proyecciones
      const tableData = proyecciones.map((p) => [
        p.ciclo.toString(),
        p.mes.toString(),
        formatCurrency(p.ingresosBrutos),
        formatCurrency(p.costosAlimento),
        formatCurrency(p.costosCal),
        formatCurrency(p.ingresoNeto),
        formatCurrency(p.flujoAcumulado),
        `${p.roi.toFixed(1)}%`,
      ])
    ...
    // Guardar el PDF
    doc.save(`analisis-rentabilidad-${new Date().toISOString().split("T")[0]}.pdf`)
  }
